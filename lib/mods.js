module.exports = function()
{
	this.modules = {};
	this.stack = [];

	this.max_depth = 32;

	this.load = function(module, name)
	{
		if(typeof module === 'string')
		{
			delete require.cache[require.resolve(module)];
			module = require(module);
		}

		if(typeof module !== 'object')
		{
			throw module.exports.Error('module must be object or resolve to object', 'type');
			return;
		}

		if(name !== undefined)
		{
			module.name = name;
		}

		if(typeof module.name !== 'string')
		{
			throw module.exports.Error('module.name must be string', 'type');
			return;
		}

		if(this.modules[module.name] !== undefined)
		{
			throw module.exports.Error('module.name already exists', 'exists');
			return;
		}

		this.modules[module.name] = module;

		var args = ['$load', module.name].concat(Array.prototype.slice.call(arguments, 2));
		this.fire.apply(this, args);
	}

	this.unload = function(name)
	{
		if(this.modules[name] === undefined)
		{
			throw module.exports.Error('name does not exist', 'notexists');
			return;
		}

		var args = ['$unload', name].concat(Array.prototype.slice.call(arguments, 1));
		this.fire.apply(this, args);

		this.modules[name] = undefined;
	}

	// event_name, [params...]
	this.fire = function(event_name)
	{
		var ret = {};

		var params = Array.prototype.slice.call(arguments, 1);

		for(var module in this.modules)
		{
			var args = [module, event_name].concat(params);
			ret[module] = this.target.apply(this, args);
		}

		return ret;
	}

	// target_module, event_name, [params...]
	this.target = function(target_module, event_name)
	{
		var ret = undefined;

		var params = Array.prototype.slice.call(arguments, 2);

		if(this.stack.length >= this.max_depth)
		{
			throw module.exports.Error('max depth exceeded', 'resource');
			return;
		}

		if(typeof target_module !== 'string')
		{
			throw module.exports.Error('target module name must be string', 'type');
			return;
		}

		if(typeof event_name !== 'string')
		{
			throw module.exports.Error('event name must be string', 'type');
			return;
		}

		var active_module = this.stack[this.stack.length - 1];

		var s = event_name.split('$');
		if(s.length > 1)
		{
			active_module = s[0];
			event_name = s[1];
		}

		if(active_module === undefined) active_module = '';
		var sender = this.modules[active_module];

		if(typeof this.modules[target_module][active_module + '$' + event_name] === 'function')
		{
			this.stack.push(target_module);

			try
			{
				this.modules[target_module].__mods = this;
				this.modules[target_module].__sender = sender;
				ret = this.modules[target_module][active_module + '$' + event_name].apply(this.modules[target_module], params);
				this.stack.pop();
			}
			catch(err)
			{
				this.stack.pop();
				throw err;
			}
		}

		return ret;
	}
}

module.exports.Error = function(msg, type)
{
	var e = new Error(msg);
	e.type = type;
	return e;
}
